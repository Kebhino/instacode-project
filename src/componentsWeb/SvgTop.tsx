// import { motion } from "framer-motion";

// const MotionPath = motion.path;

// const SvgTop = () => {
//   const d1 =
//     "M0,256L0,224L36.9,224L36.9,96L73.8,96L73.8,0L110.8,0L110.8,64L147.7,64L147.7,64L184.6,64L184.6,224L221.5,224L221.5,192L258.5,192L258.5,192L295.4,192L295.4,224L332.3,224L332.3,288L369.2,288L369.2,288L406.2,288L406.2,192L443.1,192L443.1,288L480,288L480,96L516.9,96L516.9,256L553.8,256L553.8,64L590.8,64L590.8,128L627.7,128L627.7,192L664.6,192L664.6,32L701.5,32L701.5,288L738.5,288L738.5,256L775.4,256L775.4,224L812.3,224L812.3,160L849.2,160L849.2,256L886.2,256L886.2,0L923.1,0L923.1,192L960,192L960,32L996.9,32L996.9,256L1033.8,256L1033.8,224L1070.8,224L1070.8,96L1107.7,96L1107.7,160L1144.6,160L1144.6,128L1181.5,128L1181.5,96L1218.5,96L1218.5,224L1255.4,224L1255.4,224L1292.3,224L1292.3,96L1329.2,96L1329.2,96L1366.2,96L1366.2,320L1403.1,320L1403.1,32L1440,32L1440,0L1403.1,0L1403.1,0L1366.2,0L1366.2,0L1329.2,0L1329.2,0L1292.3,0L1292.3,0L1255.4,0L1255.4,0L1218.5,0L1218.5,0L1181.5,0L1181.5,0L1144.6,0L1144.6,0L1107.7,0L1107.7,0L1070.8,0L1070.8,0L1033.8,0L1033.8,0L996.9,0L996.9,0L960,0L960,0L923.1,0L923.1,0L886.2,0L886.2,0L849.2,0L849.2,0L812.3,0L812.3,0L775.4,0L775.4,0L738.5,0L738.5,0L701.5,0L701.5,0L664.6,0L664.6,0L627.7,0L627.7,0L590.8,0L590.8,0L553.8,0L553.8,0L516.9,0L516.9,0L480,0L480,0L443.1,0L443.1,0L406.2,0L406.2,0L369.2,0L369.2,0L332.3,0L332.3,0L295.4,0L295.4,0L258.5,0L258.5,0L221.5,0L221.5,0L184.6,0L184.6,0L147.7,0L147.7,0L110.8,0L110.8,0L73.8,0L73.8,0L36.9,0L36.9,0L0,0L0,0Z";
//   const d2 =
//     "M0,192L0,224L36.9,224L36.9,128L73.8,128L73.8,0L110.8,0L110.8,64L147.7,64L147.7,96L184.6,96L184.6,256L221.5,256L221.5,224L258.5,224L258.5,192L295.4,192L295.4,256L332.3,256L332.3,192L369.2,192L369.2,160L406.2,160L406.2,224L443.1,224L443.1,288L480,288L480,128L516.9,128L516.9,256L553.8,256L553.8,64L590.8,64L590.8,96L627.7,96L627.7,160L664.6,160L664.6,64L701.5,64L701.5,224L738.5,224L738.5,192L775.4,192L775.4,224L812.3,224L812.3,192L849.2,192L849.2,224L886.2,224L886.2,0L923.1,0L923.1,192L960,192L960,32L996.9,32L996.9,192L1033.8,192L1033.8,224L1070.8,224L1070.8,96L1107.7,96L1107.7,160L1144.6,160L1144.6,128L1181.5,128L1181.5,96L1218.5,96L1218.5,224L1255.4,224L1255.4,256L1292.3,256L1292.3,128L1329.2,128L1329.2,96L1366.2,96L1366.2,320L1403.1,320L1403.1,32L1440,32L1440,0Z";

//   return (
//     <motion.svg
//       width="100%"
//       height="200px"
//       viewBox="0 0 1200 320"
//       preserveAspectRatio="none"
//       xmlns="http://www.w3.org/2000/svg"
//     >
//       <MotionPath
//         fill="#0099ff"
//         fillOpacity="0.3"
//         initial={{ d: d1 }}
//         animate={{ d: d2 }}
//         transition={{
//           duration: 3,
//           repeat: Infinity,
//           repeatType: "reverse",
//           ease: "easeInOut",
//         }}
//       />
//     </motion.svg>
//   );
// };

// export default SvgTop;

import { useEffect, useRef } from "react";

const MatrixRain = () => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    canvas.width = window.innerWidth;
    canvas.height = 200; // wysokość fali

    const binary = "01";
    const fontSize = 16;
    const columns = Math.floor(canvas.width / fontSize);
    const drops = Array(columns).fill(1);

    const draw = () => {
      if (!ctx) return;

      // Białe tło (z lekkim przezroczystym zanikiem)
      ctx.fillStyle = "rgb(255, 255, 255)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      ctx.fillStyle = "#00aa00"; // Zielone cyfry
      ctx.font = `${fontSize}px monospace`;

      drops.forEach((y, i) => {
        const text = binary[Math.floor(Math.random() * binary.length)];
        ctx.fillText(text, i * fontSize, y * fontSize);

        // Reset na górze, jeśli spadnie poniżej canvas
        if (y * fontSize > canvas.height && Math.random() > 0.975) {
          drops[i] = 0;
        }

        drops[i] += 0.3; // Spadają o połowę wolniej
      });
    };

    const interval = setInterval(draw, 33);

    const handleResize = () => {
      canvas.width = window.innerWidth;
    };

    window.addEventListener("resize", handleResize);

    return () => {
      clearInterval(interval);
      window.removeEventListener("resize", handleResize);
    };
  }, []);

  return (
    <canvas
      ref={canvasRef}
      style={{ width: "100%", display: "block", backgroundColor: "white" }}
    />
  );
};

export default MatrixRain;
